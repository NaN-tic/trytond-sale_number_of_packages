...
class ShipmentOut:
    ...
    def _sync_inventory_to_outgoing(cls, shipments):
        ...
        for shipment in shipments:
            outgoing_moves_by_key = {}
            for move in shipment.outgoing_moves:
                if move.state == 'cancel':
                    continue
                key = shipment._sync_inventory_to_outgoing_grouping_key(move,
                    'outgoing')
                outgoing_moves_by_key.setdefault(key, []).append(move)
            ...
            for inv_move in shipment.inventory_moves:
                if inv_move.state == 'cancel':
                    continue
                key = shipment._sync_inventory_to_outgoing_grouping_key(
                    inv_move, 'inventory')
                ...
                if inv_move.package:
                    ...
                    inv_qty = Uom.compute_qty(inv_move.uom,
                        inv_move.quantity,
                        inv_move.product.default_uom, round=False)
                for out_move in available_outgoing_moves:
                    if inv_move.package:
                        if inv_move.lot:
                            out_qty = (
                                inv_move.lot.compute_number_of_packages(
                                    out_move.number_of_packages))
                        else:
                            out_qty = out_move.number_of_packages
                    else:
                        out_qty = Uom.compute_qty(out_qty.uom,
                            out_qty.quantity,
                            out_qty.product.default_uom, round=False)

                    if inv_qty < out_qty:
                        new_out_move = shipment._get_outgoing_move(
                            inv_move)
                        shipment._update_outgoing_move_quantity(
                            new_out_move, inv_qty, lot=inv_move.lot)
                        to_create.append(new_out_move)

                        pending_qty = out_qty - inv_qty
                        shipment._update_outgoing_move_quantity(
                            out_move, pending_qty, lot=inv_move.lot)
                        to_write.add(out_move)
                        inv_qty = 0
                        break
                    if inv_qty == out_qty or (
                            inv_qty > out_qty
                            and len(available_outgoing_moves) == 1):
                        shipment._update_outgoing_move(
                            out_move, inv_move, inv_qty)
                        to_write.add(out_move)
                        available_outgoing_moves.remove(out_move)
                        inv_qty = 0
                        break
                    if inv_qty > out_qty:
                        shipment._update_outgoing_move(
                            out_move, inv_move, out_qty)
                        to_write.add(out_move)
                        available_outgoing_moves.remove(out_move)
                        inv_qty -= out_qty
                if inv_qty > 0:
            ...
            for outgoing_moves in outgoing_moves_by_key.values():
                for move in outgoing_moves:
                    move.quantity = 0.0
                    if move.package:
                        move.number_of_packages = 0
                    to_write.add(move)
